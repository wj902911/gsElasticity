...
*/
template <class T>
class gsIncrementIterative
{
public:
    typedef memory::shared_ptr<gsIncrementIterative> Ptr;
    typedef memory::unique_ptr<gsIncrementIterative> uPtr;

    /// constructor without an initial guess. Assumes a zero initial guess.
    gsIncrementIterative(gsBaseAssembler<T> & assembler_);
    ...
    /// solution procedure
    void solve(gsMatrix<T> & totalsolVector);

    /// computes update or the next solution
    bool compute(gsMatrix<T> & totalsolVector);
...
#include GISMO_HPP_HEADER(gsIncrementIterative.hpp)
#endif
